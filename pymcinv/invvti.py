...
import surfpy.pymcinv.vtipost as vtipost
...
class vtih5(invbase.baseh5):
    
    def mc_inv(self, use_ref=False, ingrdfname=None, solver_type = 0, crtstd = None, wdisp = 1., cdist = 75.,rffactor = 40., ray=True, lov=True,\
        outdir = None, restart = False, vp_water=1.5, isconstrt=True, sedani = False, crtani=True, manani=True, sed_aniso=25., crt_aniso=10., man_aniso=10.,
        ...
        for grd_id in grdlst:
            ...
            if crtstd is None:
                vpr.get_paraind(mtype='vti', sed_aniso=sed_aniso, crt_aniso=crt_aniso, man_aniso=man_aniso, crtthk = None)
            else:
    ...
    def mc_inv_sta(self, use_ref=False, instafname = None, ray = True, lov=True, outdir = None, restart = False,
        crtstd = None, wdisp = 0.2, rffactor = 40., solver_type = 0, vp_water=1.5, isconstrt=True, sedani = False, crtani=True, manani=True,
        sed_aniso=25., crt_aniso=10., man_aniso=10., crt_depth = -1., mantle_depth = -1., step4uwalk=1500, numbrun=15000, subsize=1000, nprocess=None, parallel=True,
    ...
    def read_inv(self, datadir = None, ingrdfname=None, factor=1., thresh=0.5, stdfactor=2, avgqc=True, Nmax=None, Nmin=500, wtype='ray'):
        ...
        for grd_id in grdlst:
            ...
            igrd        += 1
            grp         = grd_grp[grd_id]
            if 'avg_paraval_iso' in grp.keys():
                print ('=== SKIP: Existing inversion results for grid: lon = '+str(grd_lon)+', lat = '+str(grd_lat)+', '+str(igrd)+'/'+str(Ngrd))
                continue
            ...
            if topovalue < 0.:
                if postvpr.waterdepth != -topovalue:
                    print ('waterdepth = %g, topovalue = %g' %(postvpr.waterdepth, topovalue))
            ...
            #------------------------------------------
            # isotropic
            grp.create_dataset(name = 'avg_paraval_iso', data = postvpr.avg_paraval)
            grp.create_dataset(name = 'med_paraval_iso', data = postvpr.med_paraval)
            grp.create_dataset(name = 'min_paraval_iso', data = postvpr.min_paraval)
            grp.create_dataset(name = 'sem_paraval_iso', data = postvpr.sem_paraval)
            grp.create_dataset(name = 'std_paraval_iso', data = postvpr.std_paraval)
            # VTI
            grp.create_dataset(name = 'avg_paraval_vti', data = postvpr.avg_paraval_vti)
            grp.create_dataset(name = 'med_paraval_vti', data = postvpr.med_paraval_vti)
            grp.create_dataset(name = 'min_paraval_vti', data = postvpr.min_paraval_vti)
            grp.create_dataset(name = 'sem_paraval_vti', data = postvpr.sem_paraval_vti)
            grp.create_dataset(name = 'std_paraval_vti', data = postvpr.std_paraval_vti)
            # 
            grp.create_dataset(name = 'z_ensemble', data = postvpr.z_ensemble)
            grp.create_dataset(name = 'vsv_upper_bound', data = postvpr.vs_upper_bound)
            grp.create_dataset(name = 'vsv_lower_bound', data = postvpr.vs_lower_bound)
            grp.create_dataset(name = 'vsv_std', data = postvpr.vs_std)
            grp.create_dataset(name = 'vsv_mean', data = postvpr.vs_mean)
            if ('disp_ph_ray') in grp.keys():
                grp.create_dataset(name = 'avg_ph_ray', data = postvpr.vprfwrd.data.dispR.pvelp)
                disp_min                = postvpr.dispray[postvpr.ind_min, :]
                grp.create_dataset(name = 'min_ph_ray', data = disp_min)
            if ('disp_ph_lov') in grp.keys():
                grp.create_dataset(name = 'avg_ph_lov', data = postvpr.vprfwrd.data.dispL.pvelp)
                disp_min                = postvpr.displov[postvpr.ind_min, :]
                grp.create_dataset(name = 'min_ph_lov', data = disp_min)
            grp.attrs.create(name = 'average_mod_misfit', data = postvpr.vprfwrd.data.misfit)
    ...
    def read_inv_sta(self, datadir = None, instafname=None, factor=1., thresh=0.5, stdfactor=2, avgqc=True,\
        ...
        for staid in stalst:
            ...
            if 'avg_paraval_iso' in grp.keys():
                print ('=== SKIP: Existing inversion results for station id: '+staid+', '+str(ista)+'/'+str(Nsta))
                continue
    ...
    def merge_sta_grd(self, infname = None, cdist = 75.):
        ...
        if infname is None:
            ...
            indset  = vtih5(infname)
            sta_grps= indset['sta_pts']
            stlos   = indset.attrs['stlos']
            stlas   = indset.attrs['stlas']
        Nsta        = stlos.size
        self._get_lon_lat_arr()
        for grd_id in grdlst:
            split_id= grd_id.split('_')
            ...
            std_paraval_grd     = grp['std_paraval_iso'][()]
            min_paraval_grd_vti = grp['min_paraval_vti'][()]
            avg_paraval_grd_vti = grp['avg_paraval_vti'][()]
            med_paraval_grd_vti = grp['med_paraval_vti'][()]
            sem_paraval_grd_vti = grp['sem_paraval_vti'][()]
            std_paraval_grd_vti = grp['std_paraval_vti'][()]
            
            vs_upper_bound_grd  = grp['vsv_upper_bound'][()]
            vs_lower_bound_grd  = grp['vsv_lower_bound'][()]
            vs_std_grd          = grp['vsv_std'][()]
            vs_mean_grd         = grp['vsv_mean'][()]
            if is_merge:
                # print (grd_lat, stla, grd_lon, stlo, distmin)
                # station based results
                min_paraval_sta     = grp['min_paraval_iso'][()]
                avg_paraval_sta     = grp['avg_paraval_iso'][()]
                med_paraval_sta     = grp['med_paraval_iso'][()]
                sem_paraval_sta     = grp['sem_paraval_iso'][()]
                std_paraval_sta     = grp['std_paraval_iso'][()]
                min_paraval_sta_vti = grp['min_paraval_vti'][()]
                avg_paraval_sta_vti = grp['avg_paraval_vti'][()]
                med_paraval_sta_vti = grp['med_paraval_vti'][()]
                sem_paraval_sta_vti = grp['sem_paraval_vti'][()]
                std_paraval_sta_vti = grp['std_paraval_vti'][()]
                
                vs_upper_bound_sta  = grp['vsv_upper_bound'][()]
                vs_lower_bound_sta  = grp['vsv_lower_bound'][()]
                vs_std_sta          = grp['vsv_std'][()]
                vs_mean_sta         = grp['vsv_mean'][()]
                # merged results
                ...
                min_paraval_out_vti = min_paraval_grd_vti *weight_grd + (1. - weight_grd) * min_paraval_sta_vti
                avg_paraval_out_vti = avg_paraval_grd_vti *weight_grd + (1. - weight_grd) * avg_paraval_sta_vti
                med_paraval_out_vti = med_paraval_grd_vti *weight_grd + (1. - weight_grd) * med_paraval_sta_vti
                #=================
                # min depth arrays
                #=================
                # min moho
                ...
                sem_paraval_out_vti = sem_paraval_grd_vti *weight_grd + (1. - weight_grd) * sem_paraval_sta_vti
                std_paraval_out_vti = std_paraval_grd_vti *weight_grd + (1. - weight_grd) * std_paraval_sta_vti
                ...
                min_paraval_out_vti = min_paraval_grd_vti
                avg_paraval_out_vti = avg_paraval_grd_vti
                med_paraval_out_vti = med_paraval_grd_vti
                sem_paraval_out_vti = sem_paraval_grd_vti
                std_paraval_out_vti = std_paraval_grd_vti
                vs_upper_bound_out  = vs_upper_bound_grd
                vs_lower_bound_out  = vs_lower_bound_grd
                vs_std_out          = vs_std_grd
                vs_mean_out         = vs_mean_grd
            # store merged data
            grp.create_dataset(name = 'avg_paraval_iso_jnt', data = avg_paraval_out)
            grp.create_dataset(name = 'med_paraval_iso_jnt', data = med_paraval_out)
            grp.create_dataset(name = 'min_paraval_iso_jnt', data = min_paraval_out)
            grp.create_dataset(name = 'sem_paraval_iso_jnt', data = sem_paraval_out)
            grp.create_dataset(name = 'std_paraval_iso_jnt', data = std_paraval_out)
            grp.create_dataset(name = 'avg_paraval_vti_jnt', data = avg_paraval_out_vti)
            grp.create_dataset(name = 'med_paraval_vti_jnt', data = med_paraval_out_vti)
            grp.create_dataset(name = 'min_paraval_vti_jnt', data = min_paraval_out_vti)
            grp.create_dataset(name = 'sem_paraval_vti_jnt', data = sem_paraval_out_vti)
            grp.create_dataset(name = 'std_paraval_vti_jnt', data = std_paraval_out_vti)
            grp.create_dataset(name = 'vsv_upper_bound_jnt', data = vs_upper_bound_out)
            grp.create_dataset(name = 'vsv_lower_bound_jnt', data = vs_lower_bound_out)
            grp.create_dataset(name = 'vsv_std_jnt', data = vs_std_out)
            grp.create_dataset(name = 'vsv_mean_jnt', data = vs_mean_out)
        return
    
    def get_paraval(self, pindex, dtype = 'min', mtype = 'iso', isjnt=False, ingrdfname = None, isthk = False, depth = 5., depthavg = 0.):
        """
        ...
        for grd_id in grdlst:
            ...
            grp         = grd_grp[grd_id]
            try:
                ind_lon = np.where(abs(grd_lon - self.lons_inv)< 0.01)[0][0]
                ind_lat = np.where(abs(grd_lat - self.lats_inv)< 0.01)[0][0]
            except IndexError:
                print ('ERROR in grd_pts: %s, %g, %g' %(grd_id, grd_lon, grd_lat))
                continue
            try:
                paraval                     = grp[dtype+'_paraval_'+mtype][()]
            except KeyError:
                print ('WARNING: no data at grid: lon = '+str(grd_lon)+', lat = '+str(grd_lat)+', '+str(igrd)+'/'+str(Ngrd))
                continue
            if pindex =='moho':
                if mtype == 'vti':
                    raise ValueError('Moho is stored in iso parameter array!')
    ...
    def paraval_arrays(self, igrdsta = 1, cdist = 100., dtype = 'min', isjnt=False, numbp_vti=2, sigma=1, gsigma = 50., depth = 5., depthavg = 0., verbose=False):
        """
        ...
        # isotropic paraval array
        for pindex in range(13):
            if pindex == 11:
                if igrdsta == 1:
                    data_inv= self.get_paraval(pindex = pindex, dtype = dtype, mtype = 'iso', isjnt = isjnt,\
                ...
                if igrdsta == 1:
                    data_inv= self.get_paraval(pindex = 'moho', dtype = dtype, mtype = 'iso', isjnt = isjnt,\
                                    isthk = True, depth = depth, depthavg = depthavg)
                    if np.any(data_inv[index_inv] < -100.):
                        raise ValueError('!!! Error in inverted data!')
                # # # else:
                ...
                data_smooth = smoothgrder.Zarr.copy()
            # store data
            grp.create_dataset(name = str(pindex)+'_org_iso', data = data)
            grp.create_dataset(name = str(pindex)+'_smooth_iso', data = data_smooth)
        # VTI
        for pindex in range(numbp_vti):
            if igrdsta == 1:
                data_inv= self.get_paraval(pindex = pindex, dtype = dtype, mtype = 'vti', isjnt = isjnt,\
                                isthk = False, depth = depth, depthavg = depthavg)
                if np.any(data_inv[index_inv] < -100.):
                    print (data_inv[index_inv].min(), data_inv[index_inv].max(), data_inv[index_inv].mean())
                    raise ValueError('!!! Error in inverted data!')
            ...
            data_smooth = smoothgrder.Zarr.copy()
            # store data
            grp.create_dataset(name = str(pindex)+'_org_vti', data = data)
            grp.create_dataset(name = str(pindex)+'_smooth_vti', data = data_smooth)
        return 
    
    def construct_3d(self, dtype = 'avg', is_smooth = True, maxdepth = 200., dz = 0.1):
        ...
        topo        = self['topo'][()]
        self._get_lon_lat_arr()
        if self.latArr.shape != grp['0_org_iso'][()].shape:
        ...
        for ilat in range(self.Nlat):
            for ilon in range(self.Nlon):
                ...
                for pindex in range(13):
                    if is_smooth:
                        data        = grp[str(pindex)+'_smooth_iso'][()]
                    else:
                        data        = grp[str(pindex)+'_org_iso'][()]
        ...
        if is_smooth:
            grp.create_dataset(name = 'vsv_smooth', data = vs3d)
            grp.create_dataset(name = 'z_smooth', data = zArr)
        else:
            grp.create_dataset(name = 'vsv_org', data = vs3d)
    ...
    def plot_paraval(self, pindex, is_smooth=True, dtype='avg', mtype='iso', sigma=1, gsigma = 50., \
        ...
        if pindex == 'moho':
            ...
            if is_smooth:
                ...
                data    =  self[dtype+'_paraval'+'/%d_org_%s' %(pindex, mtype)][()]
        else:
            data    = self.get_paraval(pindex = pindex, dtype = dtype, mtype = 'iso',\
                                    isthk = False, depth = depth, depthavg = depthavg)
            index_inv       = np.logical_not(self.attrs['mask_inv_result'])
            lons_inv        = self.lonArr_inv[index_inv]
            lats_inv        = self.latArr_inv[index_inv]
            ...
            gridder.read_array(inlons = lons_inv, inlats = lats_inv, inzarr = data[index_inv])
            gridder.interp_surface(do_blockmedian = True)
            data        = gridder.Zarr.copy()
        ...
        if isrelstd:
            ...
            topoArr     = self['topo'][()]
            if is_smooth:
                data2    = self['avg_paraval'+'/12_smooth_'+mtype][()]\
                            + self['avg_paraval'+'/11_smooth_'+mtype][()] - topoArr
            else:
    ...
    def plot_horizontal(self, depth, evdepavg = 5., verlats = [], verlons = [], depthb=None, depthavg=None, dtype='avg', 
        ...
        if plotfault:
            if projection == 'lambert':
                ...
                m.readshapefile(shapefname, 'faultline', linewidth = 2., color='grey', default_encoding='windows-1252')
        if plottecto:
            if os.path.isdir('/home/lili/spain_proj/geo_maps'):
                shapefname  = '/home/lili/spain_proj/geo_maps/prv4_2l-polygon'
                m.readshapefile(shapefname, 'faultline', linewidth = 2, color='grey')
